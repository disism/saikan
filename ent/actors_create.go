...
func (ac *ActorsCreate) check() error {
	return nil
}

func (ac *ActorsCreate) sqlSave(ctx context.Context) (*Actors, error) {
...
func (ac *ActorsCreate) createSpec() (*Actors, *sqlgraph.CreateSpec) {
	var (
		_node = &Actors{config: ac.config}
		_spec = sqlgraph.NewCreateSpec(actors.Table, sqlgraph.NewFieldSpec(actors.FieldID, field.TypeInt))
	)
	return _node, _spec
}
...
func (acb *ActorsCreateBulk) Save(ctx context.Context) ([]*Actors, error) {
	...
	nodes := make([]*Actors, len(acb.builders))
	mutators := make([]Mutator, len(acb.builders))
	for i := range acb.builders {
		func(i int, root context.Context) {
			builder := acb.builders[i]
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				mutation, ok := m.(*ActorsMutation)
				if !ok {
