...
func (aq *ArtistQuery) QueryMusics() *MusicQuery {
...
func (aq *ArtistQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (aq *ArtistQuery) FirstIDX(ctx context.Context) uint64 {
...
func (aq *ArtistQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (aq *ArtistQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (aq *ArtistQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (aq *ArtistQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := aq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (aq *ArtistQuery) WithMusics(opts ...func(*MusicQuery)) *ArtistQuery {
	query := (&MusicClient{config: aq.config}).Query()
...
func (aq *ArtistQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Artist, error) {
	...
	if query := aq.withMusics; query != nil {
		if err := aq.loadMusics(ctx, query, nodes,
			func(n *Artist) { n.Edges.Musics = []*Music{} },
			func(n *Artist, e *Music) { n.Edges.Musics = append(n.Edges.Musics, e) }); err != nil {
			return nil, err
		}
	}
...
func (aq *ArtistQuery) loadMusics(ctx context.Context, query *MusicQuery, nodes []*Artist, init func(*Artist), assign func(*Artist, *Music)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Artist)
	nids := make(map[uint64]map[*Artist]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(artist.MusicsTable)
		s.Join(joinT).On(s.C(music.FieldID), joinT.C(artist.MusicsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(artist.MusicsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(artist.MusicsPrimaryKey[0]))
...
func (aq *ArtistQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(artist.Table, artist.Columns, sqlgraph.NewFieldSpec(artist.FieldID, field.TypeUint64))
	_spec.From = aq.sql
