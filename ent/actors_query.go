...
// First returns the first Actors entity from the query.
// Returns a *NotFoundError when no Actors was found.
func (aq *ActorsQuery) First(ctx context.Context) (*Actors, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{actors.Label}
	}
	return nodes[0], nil
}
...
// FirstID returns the first Actors ID from the query.
// Returns a *NotFoundError when no Actors ID was found.
...
// Only returns a single Actors entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one Actors entity is found.
// Returns a *NotFoundError when no Actors entities are found.
func (aq *ActorsQuery) Only(ctx context.Context) (*Actors, error) {
	...
	case 0:
		return nil, &NotFoundError{actors.Label}
	default:
		return nil, &NotSingularError{actors.Label}
	}
}
...
// OnlyID is like Only, but returns the only Actors ID in the query.
// Returns a *NotSingularError when more than one Actors ID is found.
...
func (aq *ActorsQuery) OnlyID(ctx context.Context) (id int, err error) {
	...
	case 0:
		err = &NotFoundError{actors.Label}
	default:
		err = &NotSingularError{actors.Label}
	}
	return
}
...
func (aq *ActorsQuery) All(ctx context.Context) ([]*Actors, error) {
	...
	qr := querierAll[[]*Actors, *ActorsQuery]()
	return withInterceptors[[]*Actors](ctx, aq, qr, aq.inters)
}
...
func (aq *ActorsQuery) IDs(ctx context.Context) (ids []int, err error) {
	...
	if err = aq.Select(actors.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (aq *ActorsQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, aq, querierCount[*ActorsQuery](), aq.inters)
}
...
// Clone returns a duplicate of the ActorsQuery builder, including all associated steps. It can be
...
func (aq *ActorsQuery) Clone() *ActorsQuery {
	...
	return &ActorsQuery{
		config:     aq.config,
		ctx:        aq.ctx.Clone(),
		order:      append([]actors.OrderOption{}, aq.order...),
		inters:     append([]Interceptor{}, aq.inters...),
		predicates: append([]predicate.Actors{}, aq.predicates...),
...
func (aq *ActorsQuery) GroupBy(field string, fields ...string) *ActorsGroupBy {
	...
	grbuild := &ActorsGroupBy{build: aq}
	grbuild.flds = &aq.ctx.Fields
	grbuild.label = actors.Label
...
func (aq *ActorsQuery) Select(fields ...string) *ActorsSelect {
	...
	sbuild := &ActorsSelect{ActorsQuery: aq}
	sbuild.label = actors.Label
...
func (aq *ActorsQuery) prepareQuery(ctx context.Context) error {
...
func (aq *ActorsQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Actors, error) {
	var (
		nodes = []*Actors{}
		_spec = aq.querySpec()
	)
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*Actors).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &Actors{config: aq.config}
...
func (aq *ActorsQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := aq.querySpec()
...
func (aq *ActorsQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(actors.Table, actors.Columns, sqlgraph.NewFieldSpec(actors.FieldID, field.TypeInt))
	_spec.From = aq.sql
...
func (aq *ActorsQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(aq.driver.Dialect())
	t1 := builder.Table(actors.Table)
...
func (agb *ActorsGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*ActorsQuery, *ActorsGroupBy](ctx, agb.build, agb, agb.build.inters, v)
}

func (agb *ActorsGroupBy) sqlScan(ctx context.Context, root *ActorsQuery, v any) error {
...
func (as *ActorsSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*ActorsQuery, *ActorsSelect](ctx, as.ActorsQuery, as, as.inters, v)
}

func (as *ActorsSelect) sqlScan(ctx context.Context, root *ActorsQuery, v any) error {
