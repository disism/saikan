...
func (uq *UserQuery) QueryAlbums() *AlbumQuery {
	query := (&AlbumClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryMusics() *MusicQuery {
	query := (&MusicClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryDirs() *DirQuery {
	query := (&DirClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QuerySaves() *SavedQuery {
	query := (&SavedClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAlbums:    uq.withAlbums.Clone(),
		withMusics:    uq.withMusics.Clone(),
		withDirs:      uq.withDirs.Clone(),
		withSaves:     uq.withSaves.Clone(),
...
func (uq *UserQuery) WithAlbums(opts ...func(*AlbumQuery)) *UserQuery {
	query := (&AlbumClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithMusics(opts ...func(*MusicQuery)) *UserQuery {
	query := (&MusicClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithDirs tells the query-builder to eager-load the nodes that are connected to
// the "dirs" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithDirs(opts ...func(*DirQuery)) *UserQuery {
	query := (&DirClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithSaves(opts ...func(*SavedQuery)) *UserQuery {
	query := (&SavedClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withAlbums; query != nil {
		if err := uq.loadAlbums(ctx, query, nodes,
			func(n *User) { n.Edges.Albums = []*Album{} },
			func(n *User, e *Album) { n.Edges.Albums = append(n.Edges.Albums, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withMusics; query != nil {
		if err := uq.loadMusics(ctx, query, nodes,
			func(n *User) { n.Edges.Musics = []*Music{} },
			func(n *User, e *Music) { n.Edges.Musics = append(n.Edges.Musics, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withDirs; query != nil {
		if err := uq.loadDirs(ctx, query, nodes,
			func(n *User) { n.Edges.Dirs = []*Dir{} },
			func(n *User, e *Dir) { n.Edges.Dirs = append(n.Edges.Dirs, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withSaves; query != nil {
		if err := uq.loadSaves(ctx, query, nodes,
			func(n *User) { n.Edges.Saves = []*Saved{} },
			func(n *User, e *Saved) { n.Edges.Saves = append(n.Edges.Saves, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadAlbums(ctx context.Context, query *AlbumQuery, nodes []*User, init func(*User), assign func(*User, *Album)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.AlbumsTable)
		s.Join(joinT).On(s.C(album.FieldID), joinT.C(user.AlbumsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.AlbumsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.AlbumsPrimaryKey[0]))
...
func (uq *UserQuery) loadMusics(ctx context.Context, query *MusicQuery, nodes []*User, init func(*User), assign func(*User, *Music)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*User)
	nids := make(map[uint64]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.MusicsTable)
		s.Join(joinT).On(s.C(music.FieldID), joinT.C(user.MusicsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.MusicsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.MusicsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (uq *UserQuery) loadDirs(ctx context.Context, query *DirQuery, nodes []*User, init func(*User), assign func(*User, *Dir)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Dir(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.DirsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_dirs
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_dirs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_dirs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadSaves(ctx context.Context, query *SavedQuery, nodes []*User, init func(*User), assign func(*User, *Saved)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Saved(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SavesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_saves
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_saves" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_saves" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
