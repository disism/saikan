...
func (fq *FileQuery) QueryMusics() *MusicQuery {
	query := (&MusicClient{config: fq.config}).Query()
...
func (fq *FileQuery) QueryImages() *ImageQuery {
...
func (fq *FileQuery) QuerySaves() *SavedQuery {
	query := (&SavedClient{config: fq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := fq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (fq *FileQuery) WithMusics(opts ...func(*MusicQuery)) *FileQuery {
	query := (&MusicClient{config: fq.config}).Query()
...
func (fq *FileQuery) WithImages(opts ...func(*ImageQuery)) *FileQuery {
	query := (&ImageClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (fq *FileQuery) WithSaves(opts ...func(*SavedQuery)) *FileQuery {
	query := (&SavedClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (fq *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := fq.withMusics; query != nil {
		if err := fq.loadMusics(ctx, query, nodes,
			func(n *File) { n.Edges.Musics = []*Music{} },
			func(n *File, e *Music) { n.Edges.Musics = append(n.Edges.Musics, e) }); err != nil {
			return nil, err
		}
	}
	if query := fq.withImages; query != nil {
		if err := fq.loadImages(ctx, query, nodes,
			func(n *File) { n.Edges.Images = []*Image{} },
			func(n *File, e *Image) { n.Edges.Images = append(n.Edges.Images, e) }); err != nil {
			return nil, err
		}
	}
	if query := fq.withSaves; query != nil {
		if err := fq.loadSaves(ctx, query, nodes,
			func(n *File) { n.Edges.Saves = []*Saved{} },
			func(n *File, e *Saved) { n.Edges.Saves = append(n.Edges.Saves, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FileQuery) loadMusics(ctx context.Context, query *MusicQuery, nodes []*File, init func(*File), assign func(*File, *Music)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*File)
	for i := range nodes {
	...
	query.withFKs = true
	query.Where(predicate.Music(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(file.MusicsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.file_musics
		if fk == nil {
			return fmt.Errorf(`foreign-key "file_musics" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "file_musics" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (fq *FileQuery) loadImages(ctx context.Context, query *ImageQuery, nodes []*File, init func(*File), assign func(*File, *Image)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*File)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Image(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(file.ImagesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.file_images
		if fk == nil {
			return fmt.Errorf(`foreign-key "file_images" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "file_images" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (fq *FileQuery) loadSaves(ctx context.Context, query *SavedQuery, nodes []*File, init func(*File), assign func(*File, *Saved)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*File)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Saved(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(file.SavesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.file_saves
		if fk == nil {
			return fmt.Errorf(`foreign-key "file_saves" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "file_saves" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
