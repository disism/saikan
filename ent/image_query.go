...
func (iq *ImageQuery) QueryFile() *FileQuery {
...
func (iq *ImageQuery) QueryAlbums() *AlbumQuery {
	query := (&AlbumClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (iq *ImageQuery) QueryPlaylists() *PlaylistQuery {
	query := (&PlaylistClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (iq *ImageQuery) WithFile(opts ...func(*FileQuery)) *ImageQuery {
	query := (&FileClient{config: iq.config}).Query()
...
func (iq *ImageQuery) WithAlbums(opts ...func(*AlbumQuery)) *ImageQuery {
	query := (&AlbumClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (iq *ImageQuery) WithPlaylists(opts ...func(*PlaylistQuery)) *ImageQuery {
	query := (&PlaylistClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	...
	if query := iq.withFile; query != nil {
		if err := iq.loadFile(ctx, query, nodes, nil,
			func(n *Image, e *File) { n.Edges.File = e }); err != nil {
			return nil, err
		}
	}
	if query := iq.withAlbums; query != nil {
		if err := iq.loadAlbums(ctx, query, nodes,
			func(n *Image) { n.Edges.Albums = []*Album{} },
			func(n *Image, e *Album) { n.Edges.Albums = append(n.Edges.Albums, e) }); err != nil {
			return nil, err
		}
	}
	if query := iq.withPlaylists; query != nil {
		if err := iq.loadPlaylists(ctx, query, nodes,
			func(n *Image) { n.Edges.Playlists = []*Playlist{} },
			func(n *Image, e *Playlist) { n.Edges.Playlists = append(n.Edges.Playlists, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadFile(ctx context.Context, query *FileQuery, nodes []*Image, init func(*Image), assign func(*Image, *File)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Image)
	for i := range nodes {
		if nodes[i].file_images == nil {
			continue
		}
		fk := *nodes[i].file_images
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "file_images" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (iq *ImageQuery) loadAlbums(ctx context.Context, query *AlbumQuery, nodes []*Image, init func(*Image), assign func(*Image, *Album)) error {
	...
	query.Where(predicate.Album(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(image.AlbumsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.image_albums
		if fk == nil {
			return fmt.Errorf(`foreign-key "image_albums" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "image_albums" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (iq *ImageQuery) loadPlaylists(ctx context.Context, query *PlaylistQuery, nodes []*Image, init func(*Image), assign func(*Image, *Playlist)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Image)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Playlist(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(image.PlaylistsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.image_playlists
		if fk == nil {
			return fmt.Errorf(`foreign-key "image_playlists" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "image_playlists" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
