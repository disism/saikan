...
func (mq *MusicQuery) QueryUser() *UserQuery {
...
func (mq *MusicQuery) QueryFile() *FileQuery {
...
func (mq *MusicQuery) QueryArtists() *ArtistQuery {
	query := (&ArtistClient{config: mq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := mq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (mq *MusicQuery) QueryAlbums() *AlbumQuery {
...
func (mq *MusicQuery) FirstID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (mq *MusicQuery) FirstIDX(ctx context.Context) uint64 {
...
func (mq *MusicQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	var ids []uint64
...
func (mq *MusicQuery) OnlyIDX(ctx context.Context) uint64 {
...
func (mq *MusicQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (mq *MusicQuery) IDsX(ctx context.Context) []uint64 {
	ids, err := mq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (mq *MusicQuery) WithUser(opts ...func(*UserQuery)) *MusicQuery {
	query := (&UserClient{config: mq.config}).Query()
...
func (mq *MusicQuery) WithFile(opts ...func(*FileQuery)) *MusicQuery {
	query := (&FileClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MusicQuery) WithArtists(opts ...func(*ArtistQuery)) *MusicQuery {
	query := (&ArtistClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MusicQuery) WithAlbums(opts ...func(*AlbumQuery)) *MusicQuery {
	query := (&AlbumClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (mq *MusicQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Music, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, music.ForeignKeys...)
	}
	...
	if query := mq.withUser; query != nil {
		if err := mq.loadUser(ctx, query, nodes,
			func(n *Music) { n.Edges.User = []*User{} },
			func(n *Music, e *User) { n.Edges.User = append(n.Edges.User, e) }); err != nil {
			return nil, err
		}
	}
	if query := mq.withFile; query != nil {
		if err := mq.loadFile(ctx, query, nodes, nil,
			func(n *Music, e *File) { n.Edges.File = e }); err != nil {
			return nil, err
		}
	}
	if query := mq.withArtists; query != nil {
		if err := mq.loadArtists(ctx, query, nodes,
			func(n *Music) { n.Edges.Artists = []*Artist{} },
			func(n *Music, e *Artist) { n.Edges.Artists = append(n.Edges.Artists, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := mq.withAlbums; query != nil {
		if err := mq.loadAlbums(ctx, query, nodes,
			func(n *Music) { n.Edges.Albums = []*Album{} },
			func(n *Music, e *Album) { n.Edges.Albums = append(n.Edges.Albums, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MusicQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Music, init func(*Music), assign func(*Music, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Music)
	nids := make(map[uint64]map[*Music]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(music.UserTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(music.UserPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(music.UserPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(music.UserPrimaryKey[1]))
...
func (mq *MusicQuery) loadFile(ctx context.Context, query *FileQuery, nodes []*Music, init func(*Music), assign func(*Music, *File)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Music)
	for i := range nodes {
		if nodes[i].file_musics == nil {
			continue
		}
		fk := *nodes[i].file_musics
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "file_musics" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (mq *MusicQuery) loadArtists(ctx context.Context, query *ArtistQuery, nodes []*Music, init func(*Music), assign func(*Music, *Artist)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Music)
	nids := make(map[uint64]map[*Music]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(music.ArtistsTable)
		s.Join(joinT).On(s.C(artist.FieldID), joinT.C(music.ArtistsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(music.ArtistsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(music.ArtistsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (mq *MusicQuery) loadAlbums(ctx context.Context, query *AlbumQuery, nodes []*Music, init func(*Music), assign func(*Music, *Album)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Music)
	nids := make(map[uint64]map[*Music]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(music.AlbumsTable)
		s.Join(joinT).On(s.C(album.FieldID), joinT.C(music.AlbumsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(music.AlbumsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(music.AlbumsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (mq *MusicQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(music.Table, music.Columns, sqlgraph.NewFieldSpec(music.FieldID, field.TypeUint64))
	_spec.From = mq.sql
