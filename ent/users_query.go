...
func (uq *UsersQuery) QueryDevices() *DevicesQuery {
	query := (&DevicesClient{config: uq.config}).Query()
...
func (uq *UsersQuery) QueryPlaylists() *PlaylistsQuery {
	query := (&PlaylistsClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UsersQuery) QueryAlbums() *AlbumsQuery {
	query := (&AlbumsClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UsersQuery) QueryMusics() *MusicsQuery {
	query := (&MusicsClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UsersQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Users, error) {
	...
	if query := uq.withDevices; query != nil {
		if err := uq.loadDevices(ctx, query, nodes,
			func(n *Users) { n.Edges.Devices = []*Devices{} },
			func(n *Users, e *Devices) { n.Edges.Devices = append(n.Edges.Devices, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withPlaylists; query != nil {
		if err := uq.loadPlaylists(ctx, query, nodes,
			func(n *Users) { n.Edges.Playlists = []*Playlists{} },
			func(n *Users, e *Playlists) { n.Edges.Playlists = append(n.Edges.Playlists, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withAlbums; query != nil {
		if err := uq.loadAlbums(ctx, query, nodes,
			func(n *Users) { n.Edges.Albums = []*Albums{} },
			func(n *Users, e *Albums) { n.Edges.Albums = append(n.Edges.Albums, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withMusics; query != nil {
		if err := uq.loadMusics(ctx, query, nodes,
			func(n *Users) { n.Edges.Musics = []*Musics{} },
			func(n *Users, e *Musics) { n.Edges.Musics = append(n.Edges.Musics, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UsersQuery) loadDevices(ctx context.Context, query *DevicesQuery, nodes []*Users, init func(*Users), assign func(*Users, *Devices)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Users)
	for i := range nodes {
	...
	query.withFKs = true
	query.Where(predicate.Devices(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(users.DevicesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.users_devices
		if fk == nil {
			return fmt.Errorf(`foreign-key "users_devices" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "users_devices" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UsersQuery) loadPlaylists(ctx context.Context, query *PlaylistsQuery, nodes []*Users, init func(*Users), assign func(*Users, *Playlists)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Users)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Playlists(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(users.PlaylistsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.users_playlists
		if fk == nil {
			return fmt.Errorf(`foreign-key "users_playlists" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "users_playlists" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UsersQuery) loadAlbums(ctx context.Context, query *AlbumsQuery, nodes []*Users, init func(*Users), assign func(*Users, *Albums)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Users)
	nids := make(map[uint64]map[*Users]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(users.AlbumsTable)
		s.Join(joinT).On(s.C(albums.FieldID), joinT.C(users.AlbumsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(users.AlbumsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(users.AlbumsPrimaryKey[0]))
...
func (uq *UsersQuery) loadMusics(ctx context.Context, query *MusicsQuery, nodes []*Users, init func(*Users), assign func(*Users, *Musics)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Users)
	nids := make(map[uint64]map[*Users]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(users.MusicsTable)
		s.Join(joinT).On(s.C(musics.FieldID), joinT.C(users.MusicsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(users.MusicsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(users.MusicsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (uq *UsersQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(users.Table, users.Columns, sqlgraph.NewFieldSpec(users.FieldID, field.TypeUint64))
	_spec.From = uq.sql
