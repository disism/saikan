...
func (dq *DevicesQuery) QueryUser() *UsersQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(devices.Table, devices.FieldID, selector),
...
// First returns the first Devices entity from the query.
// Returns a *NotFoundError when no Devices was found.
func (dq *DevicesQuery) First(ctx context.Context) (*Devices, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{devices.Label}
	}
	return nodes[0], nil
}
...
// FirstID returns the first Devices ID from the query.
// Returns a *NotFoundError when no Devices ID was found.
...
// Only returns a single Devices entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one Devices entity is found.
// Returns a *NotFoundError when no Devices entities are found.
func (dq *DevicesQuery) Only(ctx context.Context) (*Devices, error) {
	...
	case 0:
		return nil, &NotFoundError{devices.Label}
	default:
		return nil, &NotSingularError{devices.Label}
	}
}
...
// OnlyID is like Only, but returns the only Devices ID in the query.
// Returns a *NotSingularError when more than one Devices ID is found.
...
func (dq *DevicesQuery) OnlyID(ctx context.Context) (id uint64, err error) {
	...
	case 0:
		err = &NotFoundError{devices.Label}
	default:
		err = &NotSingularError{devices.Label}
	}
	return
}
...
func (dq *DevicesQuery) All(ctx context.Context) ([]*Devices, error) {
	...
	qr := querierAll[[]*Devices, *DevicesQuery]()
	return withInterceptors[[]*Devices](ctx, dq, qr, dq.inters)
}
...
func (dq *DevicesQuery) IDs(ctx context.Context) (ids []uint64, err error) {
	...
	if err = dq.Select(devices.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (dq *DevicesQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, dq, querierCount[*DevicesQuery](), dq.inters)
}
...
// Clone returns a duplicate of the DevicesQuery builder, including all associated steps. It can be
...
func (dq *DevicesQuery) Clone() *DevicesQuery {
	...
	return &DevicesQuery{
		config:     dq.config,
		ctx:        dq.ctx.Clone(),
		order:      append([]devices.OrderOption{}, dq.order...),
		inters:     append([]Interceptor{}, dq.inters...),
		predicates: append([]predicate.Devices{}, dq.predicates...),
...
func (dq *DevicesQuery) GroupBy(field string, fields ...string) *DevicesGroupBy {
	...
	grbuild := &DevicesGroupBy{build: dq}
	grbuild.flds = &dq.ctx.Fields
	grbuild.label = devices.Label
...
func (dq *DevicesQuery) Select(fields ...string) *DevicesSelect {
	...
	sbuild := &DevicesSelect{DevicesQuery: dq}
	sbuild.label = devices.Label
...
func (dq *DevicesQuery) prepareQuery(ctx context.Context) error {
...
func (dq *DevicesQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Devices, error) {
	var (
		nodes       = []*Devices{}
		withFKs     = dq.withFKs
		_spec       = dq.querySpec()
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, devices.ForeignKeys...)
	}
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*Devices).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &Devices{config: dq.config}
	...
	if query := dq.withUser; query != nil {
		if err := dq.loadUser(ctx, query, nodes, nil,
			func(n *Devices, e *Users) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DevicesQuery) loadUser(ctx context.Context, query *UsersQuery, nodes []*Devices, init func(*Devices), assign func(*Devices, *Users)) error {
	ids := make([]uint64, 0, len(nodes))
	nodeids := make(map[uint64][]*Devices)
	for i := range nodes {
		if nodes[i].users_devices == nil {
			continue
		}
		fk := *nodes[i].users_devices
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "users_devices" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (dq *DevicesQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := dq.querySpec()
...
func (dq *DevicesQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(devices.Table, devices.Columns, sqlgraph.NewFieldSpec(devices.FieldID, field.TypeUint64))
	_spec.From = dq.sql
...
func (dq *DevicesQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(dq.driver.Dialect())
	t1 := builder.Table(devices.Table)
...
func (dgb *DevicesGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*DevicesQuery, *DevicesGroupBy](ctx, dgb.build, dgb, dgb.build.inters, v)
}

func (dgb *DevicesGroupBy) sqlScan(ctx context.Context, root *DevicesQuery, v any) error {
...
func (ds *DevicesSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*DevicesQuery, *DevicesSelect](ctx, ds.DevicesQuery, ds, ds.inters, v)
}

func (ds *DevicesSelect) sqlScan(ctx context.Context, root *DevicesQuery, v any) error {
