...
func (tx *Tx) init() {
	tx.Actors = NewActorsClient(tx.config)
	tx.Albums = NewAlbumsClient(tx.config)
	tx.Artists = NewArtistsClient(tx.config)
	tx.Audiobooks = NewAudiobooksClient(tx.config)
	tx.Categorys = NewCategorysClient(tx.config)
	tx.Devices = NewDevicesClient(tx.config)
	tx.Directors = NewDirectorsClient(tx.config)
	tx.Files = NewFilesClient(tx.config)
	tx.Images = NewImagesClient(tx.config)
	tx.Musics = NewMusicsClient(tx.config)
	tx.Oidc = NewOidcClient(tx.config)
	tx.Playlists = NewPlaylistsClient(tx.config)
	tx.Users = NewUsersClient(tx.config)
	tx.Videos = NewVideosClient(tx.config)
}
...
// applies a query, for example: Actors.QueryXXX(), the query will be executed
