...
func (e AlbumEdges) ImageOrErr() (*Image, error) {
	if e.loadedTypes[0] {
		if e.Image == nil {
...
func (e AlbumEdges) MusicsOrErr() ([]*Music, error) {
	if e.loadedTypes[1] {
		return e.Musics, nil
	}
...
// was not loaded in eager-loading.
func (e AlbumEdges) UsersOrErr() ([]*User, error) {
	if e.loadedTypes[2] {
		return e.Users, nil
	}
...
func (*Album) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case album.FieldTitle, album.FieldDescription:
			values[i] = new(sql.NullString)
		case album.FieldCreateTime, album.FieldUpdateTime:
			values[i] = new(sql.NullTime)
		case album.ForeignKeys[0]: // image_albums
			values[i] = new(sql.NullInt64)
		default:
...
func (a *Album) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case album.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field image_albums", value)
			} else if value.Valid {
				a.image_albums = new(uint64)
				*a.image_albums = uint64(value.Int64)
			}
		default:
...
// QueryImage queries the "image" edge of the Album entity.
func (a *Album) QueryImage() *ImageQuery {
	return NewAlbumClient(a.config).QueryImage(a)
}

// QueryMusics queries the "musics" edge of the Album entity.
func (a *Album) QueryMusics() *MusicQuery {
	return NewAlbumClient(a.config).QueryMusics(a)
}

// QueryUsers queries the "users" edge of the Album entity.
func (a *Album) QueryUsers() *UserQuery {
	return NewAlbumClient(a.config).QueryUsers(a)
}
